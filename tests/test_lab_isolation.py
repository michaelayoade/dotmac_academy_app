...
def test_lab_instance_invisible_across_tenants(admin_session, app_user_session, tenant_a, tenant_b):
    c = Course(
    ...
    a = Activity(
        tenant_id=tenant_a.id, course_id=c.id, chapter_number=14, type="lab", title="VLAN lab", pass_threshold=0.6
    )
    admin_session.add(a)
    admin_session.flush()

    p = Person(
        tenant_id=tenant_a.id, email=f"learner-{uuid.uuid4().hex[:8]}@a.example.com", first_name="Lab", last_name="Rat"
    )
    admin_session.add(p)
    admin_session.flush()
    ...
    li = LabInstance(
        ...
        activity_id=a.id,
        person_id=p.id,
        instance_name=f"dal-{uuid.uuid4().hex[:8]}",
        seed={"o": 5},
        status="queued",
        consoles={},
    )
    ...
    app_user_session.execute(text(f"SET app.current_tenant = '{tenant_b.id}'"))
    rows = app_user_session.execute(text("SELECT count(*) FROM lab_instances")).scalar()
