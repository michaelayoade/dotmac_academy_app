...
| Learning state for staff and external learners (ADR 0004, proposed) | Academy curriculum, enrolment, activity, assessment, completion | This Academy — sole LMS; `dotmac_erp`'s `training_course`/`lesson`/`assessment` tree is non-authoritative | ERP `TrainingCourseAssignment` + `EmployeeCertification` projections | Webhook projection is idempotent and rebuildable from Academy state; unmatched identity must fail loudly, never record a false success |
| Employment, training requirement and credential state (ADR 0004, proposed) | ERP `Employee`, department/manager, assigned training | `dotmac_erp` | Academy `Enrollment.audience` + `employee_ref` supplied at enrolment | Identity is the employee reference, never a lowercased email match |
